...
export const getAllAddresses = createAsyncThunk(
  ...
  async () => {
    try {
      const res = await GetHook("/api/users/addresses");
      return res;
    } catch (error) {
...
// Get Addresse By Id
export const getAddresseById = createAsyncThunk(
  "Addresses/getbyid",
  async (id: string) => {
    try {
...
//Create Addresse
export const createAddresse = createAsyncThunk(
  "Addresses/create",
  async (AddresseData: CreateAddresse) => {
    try {
      const data = await PostHook(`/api/users/addresses`, AddresseData);
      if (data) {
        toast.success(
          `Create Addresse ${data?.Addresse?.name} Account Successfully`
        );
      }
      return data;
    } catch (error) {
...
export const updateAddresse = createAsyncThunk(
  "Addresses/update",
  async (AddresseData: UpdateAddresse) => {
    try {
      const data = await PostHook(
        `/api/users/addresses/${AddresseData?.id}`,
        AddresseData
      );
      if (data) {
