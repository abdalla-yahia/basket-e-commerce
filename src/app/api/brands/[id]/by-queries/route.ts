...
export async function GET(request:NextRequest,{params}:{params:Promise<{id:string}>}):Promise<NextResponse>{
    try {
        ...
        if(searchText){
            products = await prisma.brand.findUnique(
                {where:{id:id},
                select:{
                    products:{
        ...
        return NextResponse.json({message:"Get Products Of brand Successfully",products,pages,status:200},{status:200})
    } catch (error) {
        return NextResponse.json({message:'Faild To Get Products On brand',error},{status:500})
    }
}