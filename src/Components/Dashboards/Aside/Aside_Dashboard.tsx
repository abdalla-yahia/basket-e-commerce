    ...
    <aside className="w-[15%]  flex flex-col justify-start items-start rounded   text-[#3E445A] font-[400] text-sm gap-0 border border-[#E4E5EE]">
          {
            role === 'users' ? (dashboardIcons?.users?.map(icon=>{
              return <div key={icon?.id} className='w-full hover:bg-primary hover:text-white duration-100 cursor-pointer p-3  flex justify-between items-start gap-3'>
                <Link href={icon?.href}>{icon?.title}</Link>
