      ...
      <div className="w-full flex items-stretch justify-start h-full flex-wrap">
        {/*Items*/}
        {
          AllCategories?.categories && AllCategories?.categories?.slice(0, 8)?.map((category: UpdateCategory) =>
            <Link key={category?.id} href={`/products/categories/${category?.id}`} className="min-w-1/4 min-h-1/2 flex-1 flex justify-start items-center gap-2 p-5 border border-[#E4E5EE]">
