...
export default function Search_Section({ ProductsCount }: { ProductsCount: number }) {
  ...
  return (
    <div className="w-full flex justify-between items-center p-5 gap-5  rounded-lg bg-[#F7F8FD]">
      ...
      {/*Search Input*/}
      <input onChange={(e) => dispatch(setSearchTextRedux(e.target.value))} type="text" name="" id="" className="outline-none border-none h-full flex-1" placeholder="Search For Product..." />
      {/*Filter Option*/}
